...
export default async function DealerLandingPage({ params }: DealerPageProps) {
  ...
  return (
    ...
    >
      ...
      >
        <strong style={{ fontSize: 14 }}>{dealerShortName}</strong>
        <nav style={{ display: "flex", gap: 16, fontSize: 13, fontWeight: 900 }}>
          <span>New</span>
          <span>Used</span>
          <span>Service</span>
          <span style={{ color: brandColor }}>Trade</span>
        </nav>
      </header>
      ...
      <section
        style={{
          ...
          background: `radial-gradient(circle at 50% 0%, ${hexToRgba(brandColor, 0.13)}, transparent 34%), linear-gradient(180deg, #ffffff 0%, #f5f7fb 100%)`,
          textAlign: "center",
        }}
      >
        <div style={{ maxWidth: 1040, margin: "0 auto" }}>
          ...
          <a
            ...
            style={{
              ...
              fontWeight: 900,
              textDecoration: "none",
              boxShadow: `0 18px 42px ${hexToRgba(brandColor, 0.24)}`,
            }}
          >
            Start Vehicle Scan
          </a>
      ...
      >
        {dealer.name} • Powered by TradeDesk by Solace
      </footer>
    </main>
  );
}
...
function hexToRgba(hex: string, alpha: number) {
  ...
  if (!Number.isFinite(number)) return `rgba(185,28,28,${alpha})`;
